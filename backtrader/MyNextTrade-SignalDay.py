...
class St(bt.Strategy):
    ...
    def logdata(self):
        ...
        txt.append("{:.2f}".format(self.st_24[0]))
        txt.append("{:.2f}".format(self.st_30[0]))
        print(", ".join(txt))
    ...
    def logsignaldata(self):
        ...
        txt.append("{:.2f}".format(self.st_24[-1]))
    ...
    def next(self):
        ...
        date = self.data.datetime.datetime()
        st_24_1 = round(self.st_24[0], 2)
        st_24_2 = round(self.st_24[-1],2)

        st_30_1 = round(self.st_30[0], 2)
        st_30_2 = round(self.st_30[-1], 2)
        ...
        if self.my_signal_24_P1 or self.my_signal_30_P1:
            ...
            row = ["P1-CROSSDOWN80", ]
            self.sip = round(self.data.close[0] + (self.data.close[0] - self.data.low[0]), 2)
            self.smart_stop = round(self.data.low[0] - ((self.data.high[0] - self.data.low[0]) / 2),2)

            self.slippage = round((self.sip * 30) / 12000)

            print('P1 -Setup1 - Buy: T-DAYOPEN > SIP {}: BUY:{}~{}, STOP: {}'.format(self.sip,self.sip,self.sip+self.slippage, self.smart_stop))
            print('P1 - Setup2 - Sell: T-DAYOPEN < SIP {}: SELL {}~{}, STOP: {}'.format(self.sip,self.sip,self.sip-self.slippage,
        ...
        if self.my_signal_24_P2 or self.my_signal_30_P2:
            ...
            self.slippage = round((self.data.close[0] * 40) / 12000)
            self.smart_stop = round((self.data.high[0] + self.data.low[0]) / 2,2)
            print('P2 - Setup1 - Sell: T-DAYOPEN < LOW {}: SELL: T-DAYOPEN~{}, STOP: {}'.format(self.data.low[0],self.data.close[0]-self.slippage,self.smart_stop))

            sip_sell = round((self.data.close[0] + self.data.high[0]) / 2,2)
            sip_buy = round((self.data.close[0] + self.data.low[0]) / 2,2)

            self.slippage = round((self.data.high[0] * 40) / 12000)
            p2_slip = round((self.data.close[0] * 10) / 12000)
            print('P2 - Setup2 - Sell: T-DAYOPEN > LOW {}: SELL: {}~{}, STOP: {}'.format(self.data.low[0],sip_sell,sip_sell-p2_slip,self.data.high[0]+self.slippage))

            print("OR - Cancel one after order triggered")
            self.slippage = round((self.data.low[0] * 40) / 12000)
            print('P2 - Setup2 -  Buy: T-DAYOPEN > LOW {}: BUY: {}~{}, STOP: {}'.format(self.data.low[0], sip_buy,sip_buy+p2_slip,self.data.low[0]-self.slippage))
        ...
        if self.my_signal_24_P3 or self.my_signal_30_P3:
            ...
            self.slippage = round((self.data.low[0] * 30) / 12000)
            self.smart_stop = round((self.data.high[0] +self.data.low[0]) / 2,2)
            print('P3 - Setup1 - Sell: T-DAYOPEN < LOW {}: SELL: T-DAYOPEN~{}, STOP: {}'.format(self.data.low[0],self.data.low[0]-self.slippage, self.smart_stop))

            self.smart_stop = round(self.data.close[0] - ((self.data.high[0] - self.data.low[0]) / 2),2)
            print('P3 - Setup2 - Buy:  T-DAYOPEN > CLOSE {}: BUY: {}, STOP: {}'.format(self.data.close[0],self.data.close[0],self.smart_stop))

            self.sip = round(self.data.close[0] - ((self.data.high[0] - self.data.low[0]) / 2),2)
            self.slippage = round((self.sip * 30) / 12000)
            tp = round(self.data.close[0] + ((self.data.high[0] - self.data.low[0]) / 2),2)
            print('P3 - Setup3 - Buy: CLOSE {} > T-DAYOPEN < LOW {} : BUY: {}, STOP: {}, TP: Atclose or {}'.format(self.data.close[0],self.data.low[-1], self.sip,self.sip-self.slippage, tp))
        ...
        if self.my_signal_24_P4 or self.my_signal_30_P4:
            ...
            print('P4 - Setup2 - Sell: T-DAYOPEN  < CLOSE {}: SELL: T-DAYOPEN~{}, STOP: {}'.format(self.data.close[0],self.data.close[0]-self.slippage,self.data.close[0]+p4_slip))

            self.smart_stop = round(self.data.close[0] - ((self.data.high[0] - self.data.low[0]) / 2), 2)
            print('P4 - Setup3 - Sell: T-DAYOPEN > HIGH {}: SELL: {}, STOP: {}'.format(self.data.high[0],self.data.close[0],self.smart_stop))
        ...
        if self.my_signal_24_P5 or self.my_signal_30_P5:
            ...
            print('P5 - Setup1 - Sell: T-DAYOPEN < CLOSE {}: SELL: T-DAYOPEN~T-DAYOPEN-, STOP: {}'.format(self.data.close[0],self.slippage, self.data.high[0]))

            self.slippage = round((self.data.close[0] * 45) / 12000)
            self.sip = round(self.data.close[0] +self.slippage, 2)

            self.slippage = round((self.data.close[0] * 40) / 12000)
            self.smart_stop =round(self.data.close[0] - self.slippage,2)
            print('P5 - Setup2 - Buy:  SIP {} > T-DAYOPEN > CLOSE {}: BUY: {}, STOP: {}'.format(self.sip,self.data.close[0],self.data.close[0],self.smart_stop))

            self.slippage = round((self.data.close[0] * 45) / 12000)
            self.sip = round(self.data.close[0] + self.slippage, 2)
            p5_slip = round((self.data.open[0] * 40) / 12000)
            print('P5 - Setup3 - Sell:  T-DAYOPEN > SIP {} : SELL: T-DAYOPEN, STOP: T-DAYOPEN+{}'.format(self.sip,p5_slip))
        ...
        if self.my_signal_24_P6 or self.my_signal_30_P6:
            ...
            self.smart_stop = round(self.data.close[0] - ((self.data.high[0] - self.data.low[0]) / 2), 2)
            self.sip = round(self.data.close[0] + ((self.data.high[0] - self.data.low[0]) / 2), 2)
            self.slippage = round((self.data.close[0] * 20) / 12000)
            print('P6 - Setup1 - Buy: T-DAYOPEN > CLOSE {}: BUY: {}, STOP: {}'.format(self.data.close[0],self.data.close[0],self.smart_stop))

            print('P6 - Setup2 - Sell: T-DAYOPEN < CLOSE {}: SELL: T-DAYOPEN~{}, STOP: {}'.format(self.data.close[0],self.data.close[0]-self.slippage,self.sip))
...
def run():
    ...
    if index in('IBGB100'):
        ...
        tz = 'EST5EDT'
        # tz = 'UTC'
        sessionstart = datetime.time(9, 00, 00)
